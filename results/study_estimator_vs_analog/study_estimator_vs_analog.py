...
# ---------------------------------------------------------------------------
N_THREADS = 44
photons_estimator = [1_000, 5_000, 10_000, 50_000, 100_000, 200_000]
photons_analog = [1_000_000, 5_000_000, 10_000_000, 100_000_000, 500_000_000, 1_000_000_000]
...
# Corrida
# ===========================================================================
def run_cbs(exp, radius, n_photons, estimator: bool):
    ...
    print("mode:", "estimator" if estimator else "analog", "| n_photons:", n_photons, "| runtime_s:", time.time() - t0, "| hits:", det.hits)
...
for index, rad in enumerate(radius_values):
    for n in photons_estimator:
        name = f"radius_{rad:.2f}_estimator_{n}"
        sweep.run(index, name, lambda exp, rad=rad, n=n: run_cbs(exp, rad, n, estimator=True))
    for n in photons_analog:
        name = f"radius_{rad:.2f}_analog_{n}"
        sweep.run(index, name, lambda exp, rad=rad, n=n: run_cbs(exp, rad, n, estimator=False))

...
AngularIntensity compute_speckle(Detector& detector, const int n_theta, const int n_phi) {
  ...
  for (auto &ph : detector.recorded_photons) {
    ...
    E_x(itheta, iphi) += Em_local_photon * detector.m_polarization.x + En_local_photon * detector.n_polarization.x;
    E_y(itheta, iphi) += Em_local_photon * detector.m_polarization.y + En_local_photon * detector.n_polarization.y;
    E_z(itheta, iphi) += Em_local_photon * detector.m_polarization.z + En_local_photon * detector.n_polarization.z;
  }

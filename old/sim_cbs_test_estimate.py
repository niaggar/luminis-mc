...
# ---------------------------------------------------------------------------
exp_name = "cbs_test"
base_dir = "/home/niaggar/Documents/Thesis/Tests"
...
# ---------------------------------------------------------------------------
params_sweep = [
    {"radius": 0.110, "volume_fraction": 0.1},
]

volume_fraction_a = 0.05
volume_fraction_b = 0.2
...
# ---------------------------------------------------------------------------
theta_max_far_field = np.deg2rad(2)
...
def run_single_simulation(exp, radius, volume_fraction):
    ...
    inv_mean_free_path_b = 1.0 / mean_free_path_b
    ...
    laser = Laser(
        ...
        wavelength, laser_radius * mean_free_path_a, laser_type,
    )

    anysotropy = phase.get_anisotropy_factor()[0]
    transport_mean_free_path = mean_free_path_a / (1.0 - anysotropy)
    ...
    print(f"camino libre medio l_s: {mean_free_path_a:.3f} um")
    ...
    exp.log_params(
        ...
        mu_scattering_um_inv=mu_scattering_a,
        mu_absortion_um_inv=mu_absortion_a,
        anisotropy_factor=anysotropy,
        size_parameter=size_parameter,
        mean_free_path_ls_um=mean_free_path_a,
